...
bool vtkFitsReader::DownloadSurveyDataCube(std::string str_u)
{
    ...
    const char* num = doc2.FirstChildElement("results")->FirstChildElement("DatacubeCount")->GetText();
    auto* node = doc2.FirstChildElement("results")->FirstChildElement("URL");
...
*/
    ...
    while (!found) {
        ...
        if (str_sp != "Continuum") //&&(ov==3))
        {
            ...
            found = DownloadSurveyDataCube(str_u);
            if (!found) {
                std::cout << "Actually no datacube" << std::endl;
...
bool vtkFitsReader::DownloadFITSFromUrl(std::string url)
{

    bool res = DownloadFile(url, m_fitsFile_temp);
    if (res)
...
//----------------------------------------------------------------------------
bool vtkFitsReader::DownloadFile(std::string url, std::string outName)
{
    CURL* curl;
    FILE* fp;
    CURLcode res;
    ...
    if (curl) {
        ...
        //curl_easy_setopt(curl, CURLOPT_XOAUTH2_BEARER, m_token.c_str());
